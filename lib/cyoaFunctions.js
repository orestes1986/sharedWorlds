...
// 					item.parent.0.parentid = "Constant";
					...
					}); // calling: ~/sharedworlds/shared/constantpagemain.js - addConstant/~ //
					//
...
// 					}, intervTime);
		...
		// 			console.log(error);
				...
				}); // calling: ~/sharedworlds/shared/constantpagemain.js - updateConstant/~ //
					//
		...
		removeConstant = function (item, callback) {
			Meteor.timelineFunctions.prettyConfirm('Remove item', 'Do you really want to remove item ' + item.content + '?', function (ok) {
				...
				}); // calling: ~/sharedworlds/shared/pagemain.js - removePage/~ //
					//
...
// 								arrows:{to:{scaleFactor:2}}
			...
			if (pageEvents) {
				...
				if (pageEvents[0]) {
					...
					var refreshIntervalVis = setInterval(function(){
						if ((document.getElementById('treeVisualization')) && (pageEdges) && (pageEvents) && (vis)) {
							Meteor.neTreeFunctions.network(nodes=new vis.DataSet(pageEvents), edges = new vis.DataSet(pageEdges));
							clearInterval(refreshIntervalVis);
						}
					}, intervTime);
				}
			}
